...
export class AiService {
  ...
  getConsejoExperto(hoy: number, manana: number, zona: string): Observable<string> {
    // Primero cargamos la API KEY desde el archivo local ignorado por Git
    return this.http.get<{groqApiKey: string}>('assets/data/config.json').pipe(
      catchError(() => {
        console.error('No se pudo cargar config.json. Asegúrate de que existe y tiene la clave.');
        return throwError(() => new Error('Configuración no encontrada'));
      }),
      switchMap(config => {
